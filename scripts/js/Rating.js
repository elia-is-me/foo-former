oRating = function (name) {
    ...
    this.GetIcon = function () {
        ...
        for (var i = 0; i < 3; i++) {
            ...
            this.icon[i] = image.Resize(15, 14);
            image.Dispose();
        }
        ico.Dispose();
    }
    ...
    this.OnMouse = function (event, x, y) {
        ...
        switch (event) {
            case ON_MOUSE_LBTN_DOWN:
                for (var i = 0; i < 5; i++) {
                    if (this.$.x + i * 20 < x && this.$.x + 20 * (i + 1) > x) {
                        this.rate = i + 1;
                        this.click = i;
                        this.$.Repaint();
                        break;
    ...
    this.OnPaint = function (gr) {
        ...
        for (var i = 0; i < this.rate; i++) {
            Image.Draw(gr, this.icon[1], this.$.x + i * 20, this.$.y + 8, 0, 255);
        }
        for (var j = this.rate; j < 5; j++) {
            Image.Draw(gr, this.icon[0], this.$.x + j * 20, this.$.y + 8, 0, 255);
        }
        if (this.click >= 0) {
            Image.Draw(gr, this.icon[2], this.$.x + this.click * 20, this.$.y + 8, 0, 255);
        }
    }
}
